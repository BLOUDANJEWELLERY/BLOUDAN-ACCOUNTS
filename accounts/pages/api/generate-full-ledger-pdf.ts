...
// Helper function to clean description - remove Invoice/Receipt prefixes
...
class FullLedgerPDFGenerator {
  ...
  private calculatePageConfig(): PageConfig {
    ...
    const headerSectionHeight = 150; // Reduced for full ledger (no account box)
  ...
  private drawPageHeader(page: PDFPage, data: FullLedgerPdfRequestData, pageNumber: number, totalPages: number): number {
    ...
    page.drawText("Full Ledger Statement", {
      x: MARGIN + 20,
      y: currentY - 25,
      size: 16,
      font: boldFont,
  ...
  private drawTableHeader(page: PDFPage, tableTop: number): { tableTop: number; colWidths: number[] } {
    ...
    // 10 columns with Account column added - matching your original structure
    let colWidths = [50, 80, 35, 150, 60, 60, 75, 60, 60, 75]; // Total: 705
...
let currentY = startY - ROW_OFFSET;
    ...
    entries.forEach((entry, index) => {
      ...
      const accountNo = entry.account?.accountNo || '';
      ...
      const rowData = [
        displayDate,
        accountDisplay.substring(0, 15) + (accountDisplay.length > 15 ? '...' : ''),
        entry.isOpeningBalance || entry.isClosingBalance ? 'BAL' : (entry.type || ''),
        cleanedDescription.substring(0, 35) + (cleanedDescription.length > 35 ? '...' : ''),
  ...
  private drawFooter(page: PDFPage, pageNumber: number, totalPages: number): void {
    ...
    const footerText = `Generated by ZamZam Jewellery - Full Ledger System - Page ${pageNumber} of ${totalPages}`;
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    console.log(`Generating Full Ledger PDF, entries: ${data.ledgerEntries.length}`);

    // Generate PDF
    const generator = new FullLedgerPDFGenerator();
    ...
    return res.json({ 
      success: true, 
      pdfData: pdfBase64,
      message: "Full Ledger PDF generated successfully" 
    });
